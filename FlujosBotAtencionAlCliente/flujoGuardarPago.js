    ...
    if(ctx.body.includes('_event_media_')){
        ...
        if(datosComprobante.fecha != '' && datosComprobante.fecha != undefined && datosComprobante.fecha != 'Invalid Date' && /*datosComprobante.cuenta != '' && datosComprobante.cuenta != undefined && */datosComprobante.valor != '' && datosComprobante.valor != undefined){
            ...
            if (Object.keys(datosMovimiento.data).length > 0){
                ...
                await ctxFn.provider.sendText(datosMovimiento.data[0].attributes.numeroReporta + '@s.whatsapp.net', mensajes.PAGO_VERIFICADO.replace('{MEDIO}', datosMovimiento.data[0].attributes.medio).replace('{FECHA}', new Date(datosMovimiento.data[0].attributes.fecha).format("DD-MM-YYYY hh:mm a").replace(',', '')).replace('{CUENTA}', datosMovimiento.data[0].attributes.cuenta.data.attributes.numero).replace('{TITULAR}', datosMovimiento.data[0].attributes.cuenta.data.attributes.titular).replace('{VALOR}', parseInt(datosMovimiento.data[0].attributes.valor).toLocaleString("es-CO", {style: "currency", currency: "COP", minimumFractionDigits: 0, maximumFractionDigits: 0,})).replace('{CLIENTE}', datosMovimiento.data[0].attributes.cliente.data.attributes.nombre))
