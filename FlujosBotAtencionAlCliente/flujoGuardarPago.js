...
.addAnswer(['Envíe uno por uno los pantallazos de los movimientos, tenga en cuenta que *no debe enviar el siguiente movimiento antes de recibir la notificación de movimiento guardado*'], {capture:true}, async (ctx, ctxFn) => {
    ...
    if(ctx.body.includes('_event_media_')){
        ...
        if(datosComprobante.fecha != '' && datosComprobante.fecha != undefined && datosComprobante.fecha != 'Invalid Date' && /*datosComprobante.cuenta != '' && datosComprobante.cuenta != undefined && */datosComprobante.valor != '' && datosComprobante.valor != undefined){
            ...
            if(datosComprobante.tipodocumento.includes == 'INGRESO_POR_CORRESPONSAL' || datosComprobante.tipodocumento.includes('RECARGA_DESDE') || datosComprobante.tipodocumento == 'INGRESO_DESDE_OTROS_BANCOS'){
            ...
            if (Object.keys(datosMovimiento.data).length > 0){
                ...
                await ctxFn.provider.sendText(datosMovimiento.data[0].attributes.numeroReporta + '@s.whatsapp.net', mensajes.PAGO_VERIFICADO.replace('{MEDIO}', datosMovimiento.data[0].attributes.medio).replace('{FECHA}', new Date(datosMovimiento.data[0].attributes.fecha).toLocaleString('es-ES', { day: 'numeric', month: 'long', year: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true }).replace(',', ' a las')).replace('{CUENTA}', datosMovimiento.data[0].attributes.cuenta.data.attributes.numero).replace('{TITULAR}', datosMovimiento.data[0].attributes.cuenta.data.attributes.titular).replace('{VALOR}', parseInt(datosMovimiento.data[0].attributes.valor).toLocaleString("es-CO", {style: "currency", currency: "COP", minimumFractionDigits: 0, maximumFractionDigits: 0,})).replace('{CLIENTE}', datosMovimiento.data[0].attributes.cliente.data.attributes.nombre))
