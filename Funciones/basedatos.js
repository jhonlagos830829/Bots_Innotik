 ...
 * @returns 
 */
async function Actualizar(datos, url, idDocumento){
  
  //Declaracion de variables
  const http = require('http');
  var salida
  console.log('LA URL ' + configuracion.URL_API + url + '/' + idDocumento)
  ...
  try {
    ...
    let config = {
      method: 'put',
      maxBodyLength: Infinity,
      url: configuracion.URL_API + url + '/' + idDocumento,
      headers: { 
        'Content-Type': 'application/json', 
        'Authorization': 'Bearer ' + configuracion.TOKEN_STRAPI
      },
...
async function Buscar (url, campo){
    ...
    console.log('Asi sera la consulta ' + configuracion.URL_API + url + campo)
    try {
...
module.exports = {Guardar, Actualizar, Buscar}